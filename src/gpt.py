...
def retry_arduino_connection():
    ...
    while not arduino:
        try:
            ...
            print("Failed to reconnect to Arduino, retrying...")
            time.sleep(0.5)
...
while not arduino:
    try:
        ...
        print("can't open arduino , retrying...")
        time.sleep(0.5)
...
def capture_image():
    ...
    cap = cv2.VideoCapture('http://192.168.199.49:8080/video')
    ret, frame = cap.read()
    if ret:
...
def classify_bean(image_path):
    ...
    if detected_classes and len(detected_classes[0]) > 0:
        ...
        annotated_folder = "newly_annotated"
        ...
        while True:
            ...
            while arduino_response == "READY":
                ...
                while attempts < 2 and bean_class is None:
                    ...
                    if not image_path:
                        print("Error capturing image, skipping.")
                        continue
                ...
                send_to_arduino(str(reserve_bean))
                if bean_class is None:
                    ...
                    # eject_bean = reserve_bean
                    reserve_bean = bean_class
                
                logging.info(f"current bean = {bean_class} && reserve bean = {reserve_bean} && eject bean = ")
